...
from crud import lesson_video as lesson_video_crud
from database.session import get_tenant_db
from models.chapter import Chapter
from models.lesson import Lesson
from models.lesson_video import LessonVideo
from schemas.lesson_video import CreateLessonVideo, LessonVideoResponse, UpdateLessonVideo
...
async def _get_video_or_404(db: AsyncSession, *, lesson_id: uuid.UUID) -> LessonVideo:
    video = await lesson_video_crud.get_lesson_video_by_lesson_id(db, lesson_id=lesson_id)
    if video is None:
        raise HTTPException(
...
async def get_lesson_video(
    ...
    await _get_lesson_or_404(db, chapter_id=chapter_id, lesson_id=lesson_id)
    video = await _get_video_or_404(db, lesson_id=lesson_id)
    return LessonVideoResponse.model_validate(video)


@router.post("", response_model=LessonVideoResponse, status_code=status.HTTP_201_CREATED)
async def create_lesson_video(
    ...
    existing = await lesson_video_crud.get_lesson_video_by_lesson_id(db, lesson_id=lesson_id)
